...
class LLaMAVIDMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        vggt_images=None # [新增] 接收 VGGT 图像
    ): 
        ...
        # ==========================================
        # [新增] 提取并投影 VGGT Latent Tokens
        # ==========================================
        vggt_3d_tokens_list = None
        vggt_model = getattr(self, "vggt_model", getattr(self.get_model(), "vggt_model", None))
        vggt_latent_projector = getattr(self, "vggt_latent_projector", getattr(self.get_model(), "vggt_latent_projector", None))
        ...
        if vggt_images is not None and vggt_model is not None and vggt_latent_projector is not None:
            with torch.no_grad():
                ...
                vggt_param = next(self.vggt_model.parameters())
                vggt_outputs = self.vggt_model(vggt_images_input.to(device=vggt_param.device, dtype=vggt_param.dtype))
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if v3:
                ...
                # ==========================================
                # [新增] 提取该批次的 VGGT 3D Tokens，准备注入
                # ==========================================
                vggt_3d_tokens = None
                if vggt_3d_tokens_list is not None:
                    cur_vggt_3d = vggt_3d_tokens_list[cur_image_idx]
