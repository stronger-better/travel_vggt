    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image_feature_path' in sources[0]:
            ...
            if history_waypoint.shape[0] == 0:
                history_waypoint = np.asarray(trajectory[:1, :3], dtype=np.float32)
            target_point = np.asarray(trajectory[-1, :3], dtype=np.float32)
        ...
        if 'image_feature_path' in ori_sources[0]:
            ...
            data_dict['vggt_image'] = vggt_image # [新增] 将专供 VGGT 的图像存入字典
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # ==========================================
        # [新增] VGGT 图像的 Batch 堆叠
        # ==========================================
        if 'vggt_image' in instances[0]:
            vggt_images = [instance['vggt_image'] for instance in instances]
            batch['vggt_images'] = torch.stack(vggt_images, dim=0) # 最终 shape: [B, S, C, H, W]
