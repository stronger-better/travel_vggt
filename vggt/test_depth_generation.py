...
def load_test_data(json_path, dataset_dir, depth_ext=".exr", depth_scale=0.001):
    """
    加载 test 数据集
    
    Args:
        json_path: transforms_test.json 路径
        dataset_dir: 数据集根目录
        depth_ext: 深度图扩展名
        depth_scale: 深度图缩放因子（mm 转 m）
    
    Returns:
        list: 包含 (image_path, depth_path, image, depth_map) 的列表
    """
    logger.info(f"Loading test data from: {json_path}")
    ...
    for frame in tqdm(frames, desc="Loading images and depth maps"):
        ...
        if os.path.isabs(file_path):
            ...
            # 处理相对路径（如 ../../test/block_1_test/0001.png）
            path_parts = [p for p in file_path.split("/") if p and p != ".." and p != "."]
            
            # 如果 path_parts 的第一个元素是 "test"，并且 dataset_dir 已经以 "test" 结尾，则跳过它
            # 例如：file_path = "../../test/block_2_test/0054.png"
            #      path_parts = ['test', 'block_2_test', '0054.png']
            #      dataset_dir = ".../small_city/test"
            #      应该得到 ".../small_city/test/block_2_test/0054.png" 而不是 ".../small_city/test/test/block_2_test/0054.png"
            if len(path_parts) > 0 and path_parts[0] == "test":
                # 检查 dataset_dir 是否以 "test" 结尾
                dataset_dir_normalized = os.path.normpath(dataset_dir)
                if dataset_dir_normalized.endswith("test") or dataset_dir_normalized.endswith(os.path.join("", "test")):
                    # 跳过 path_parts 中的第一个 "test"
                    path_parts = path_parts[1:]
            
            if len(path_parts) >= 2 and path_parts[0] == path_parts[1]:
                # 嵌套结构：block_1/block_1/0001.png
                image_path = os.path.join(dataset_dir, *path_parts)
            else:
                # 扁平结构：block_1/0001.png
                image_path = os.path.join(dataset_dir, *path_parts)
        
        # 尝试多个可能的路径
        # 处理 ../../test/block_1_test/0001.png 这种情况
        file_path_clean = file_path.replace("../", "").replace("./", "")
        if file_path_clean.startswith("/"):
            file_path_clean = file_path_clean[1:]
        
        # 如果 file_path_clean 以 "test/" 开头，且 dataset_dir 以 "test" 结尾，则移除 "test/"
        if file_path_clean.startswith("test/") and (dataset_dir.endswith("test") or dataset_dir.endswith(os.path.join("", "test"))):
            file_path_clean = file_path_clean[5:]  # 移除 "test/"
        
        image_path_candidates = [
            image_path,  # 之前计算的路径（已经处理了 test 重复问题）
            os.path.join(dataset_dir, file_path_clean),  # 清理后的相对路径
            os.path.join(dataset_dir, *path_parts),  # 使用处理后的 path_parts
        ]
        
        # 移除 None 值和重复值
        image_path_candidates = list(dict.fromkeys([c for c in image_path_candidates if c is not None]))
        ...
        if image_path is None:
            ...
            logger.warning(f"  Tried candidates: {image_path_candidates[:3]}...")
            continue
        ...
        depth_fname = img_fname_base + depth_ext
        
        # 尝试多个可能的深度图路径
        parent_dir = os.path.dirname(img_dir)
        depth_dir_name = os.path.basename(img_dir) + "_depth"
        depth_path_candidates = [
            os.path.join(parent_dir, depth_dir_name, depth_fname),
            os.path.join(img_dir + "_depth", depth_fname),
            os.path.join(parent_dir, depth_dir_name, depth_dir_name, depth_fname),
        ]
        
        depth_path = None
        for candidate in depth_path_candidates:
            if candidate and os.path.exists(candidate):
                depth_path = candidate
                break
        
        if depth_path is None:
            logger.warning(f"Depth map not found for: {image_path}")
            continue
        
        # 加载深度图
        if depth_path.lower().endswith(".npy"):
            depth_map = np.load(depth_path).astype(np.float32) * depth_scale
        else:
    ...
    if len(test_data) == 0:
        ...
        logger.warning("  1. Image paths in JSON don't match actual file locations")
        logger.warning("  2. Depth map paths don't match actual file locations")
        logger.warning("  3. Files are missing or corrupted")
        logger.warning("  4. Depth extension mismatch")
...
def main():
    parser = argparse.ArgumentParser(description="Test VGGT depth generation on test dataset")
    parser.add_argument("--test_dir", type=str, required=True,
                        help="Test dataset directory (e.g., small_city/test)")
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="Path to model checkpoint (if None, use pretrained)")
    parser.add_argument("--output_dir", type=str, default="depth_test_results",
                        help="Output directory for visualizations")
    parser.add_argument("--depth_ext", type=str, default=".exr",
                        help="Depth map extension (.exr, .png, .npy)")
    parser.add_argument("--depth_scale", type=float, default=0.001,
                        help="Depth scale factor (mm to m: 0.001)")
    ...
    # 加载测试数据
    json_path = os.path.join(args.test_dir, "transforms_test.json")
    if not os.path.exists(json_path):
        # 尝试查找 JSON 文件
        json_candidates = [
            os.path.join(args.test_dir, "transforms_test_vggt.json"),
            json_path,
        ]
        json_path = None
        ...
        if json_path is None:
            raise FileNotFoundError(f"Could not find transforms_test.json in {args.test_dir}")
    
    test_data = load_test_data(json_path, args.test_dir, args.depth_ext, args.depth_scale)
    
    if len(test_data) == 0:
        logger.error("No test data loaded! Please check:")
        logger.error(f"  1. JSON file exists: {json_path}")
        logger.error(f"  2. Image paths in JSON are correct")
        logger.error(f"  3. Depth map paths are correct")
        logger.error(f"  4. Depth extension is correct: {args.depth_ext}")
        ...
        if mask.sum() > 0:
            ...
            if pred_valid.mean() > 0 and gt_valid.mean() > 0:
                ...
                if ratio > 10 or ratio < 0.1:
                    ...
                    logger.warning(f"   1. Use --auto_scale to automatically scale predicted depth")
                    logger.warning(f"   2. Use --pred_depth_scale <factor> to manually set scale (suggested: {1/ratio:.4f})")
                    logger.warning(f"   3. Check model checkpoint depth units")
                    logger.warning(f"   4. Check GT depth scale (--depth_scale)")
